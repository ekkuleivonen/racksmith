"""SSH probing — detect hostname, OS, MAC address via SSH connection."""
...
async def probe_ssh_target(host: str, username: str, port: int) -> SSHProbeResult:
    ...
    hostname = str(hostname_result.stdout or "").strip() or host
    hostnamectl_os = _extract_hostnamectl_os(str(hostnamectl_result.stdout or ""))
    os_release = _parse_os_release(str(os_release_result.stdout or ""))
    uname = str(uname_result.stdout or "").strip()
    mac_address = _extract_mac(str(mac_result.stdout or ""))
