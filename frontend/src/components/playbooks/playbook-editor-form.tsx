...
function makeRolePickerValue(role: RoleCatalogEntry) {
  return `${role.slug}|||${role.name}|||${role.description}|||${(role.labels ?? []).join(" ")}`;
}

function parseRoleSlugFromPickerValue(value: string) {
...
export function PlaybookEditorForm({
  ...
  const rolePickerValueBySlug = useMemo(
    () =>
      Object.fromEntries(
        roles.map((role) => [role.slug, makeRolePickerValue(role)]),
      ),
    [roles],
  );

  const rolePickerItems = useMemo(
    () => Object.values(rolePickerValueBySlug),
    [rolePickerValueBySlug],
  );
    ...
    const withoutAlreadyAdded = rolesByLabel
      .map((group) => ({
        ...
        roles: group.roles.filter(
          (role) => !availableRoleSlugSet.has(role.slug),
        ),
      }))
  ...
  useEffect(() => {
    ...
    if (!draft.roles.some((role) => role.role_slug === activeRoleId)) {
      setActiveRoleId(draft.roles[0]?.role_slug);
    }
  ...
  const removeRole = (index: number) => {
    ...
    if (removedRole?.role_slug === activeRoleId) {
      const fallback = draft.roles.find((_, roleIndex) => roleIndex !== index);
      setActiveRoleId(fallback?.role_slug);
    }
  };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const oldIndex = draft.roles.findIndex(
      (role) => role.role_slug === String(active.id),
    );
    const newIndex = draft.roles.findIndex(
      (role) => role.role_slug === String(over.id),
    );
    ...
    <section className="space-y-4 border border-zinc-800 bg-zinc-900/30 p-4">
      ...
      <div className="space-y-3">
          ...
          <div ref={rolePickerAnchor} className="w-full">
            <Combobox
              ...
              onValueChange={(value) => {
                ...
                const roleSlug = parseRoleSlugFromPickerValue(value);
                addRole(roleSlug);
                setRolePickerValue(null);
                setRoleQuery("");
              }}
                  ...
                  {filteredRolesByLabel.map((group) => (
                    ...
                    >
                      ...
                      {group.roles.map((role) => {
                        return (
                          <ComboboxItem
                            key={`${group.label}-${role.slug}`}
                            value={rolePickerValueBySlug[role.slug]}
                            className="pl-4"
                          >
        ...
        {draft.roles.length === 0 ? (
          ...
          >
            <SortableContext
              items={draft.roles.map((role) => role.role_slug)}
            ...
            >
              ...
              >
                {draft.roles.map((role, index) => {
                  const roleEntry = rolesById[role.role_slug];
                  if (!roleEntry) return null;
                  return (
                    <SortableRoleCard
                      key={role.role_slug}
                      roleId={role.role_slug}
                      role={role}
                      index={index}
                      roleEntry={roleEntry}
