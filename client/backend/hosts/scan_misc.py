...
def detect_subnet() -> str:
    """Return the primary local subnet as a CIDR string (e.g. '192.168.1.0/24').

    Parses ``ip -o -4 addr`` output on Linux. Falls back to '192.168.1.0/24'.
    """
    try:
...
def arp_scan(subnet: str, timeout: int = 3) -> list[tuple[str, str]]:
    """Run an ARP scan on *subnet* using arp-scan CLI. Returns list of (ip, mac) tuples."""
    try:
        out = subprocess.check_output(
...
def lookup_vendor(mac: str) -> str:
    """Resolve a MAC address to a vendor name via OUI lookup."""
    try:
...
def lookup_vendors(macs: list[str]) -> dict[str, str]:
    """Batch vendor lookup. Returns ``{mac: vendor}``."""
    try:
        from mac_vendor_lookup import MacLookup
