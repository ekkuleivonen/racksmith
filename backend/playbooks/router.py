...
async def list_playbooks(session=Depends(auth_manager.get_current_session)):
    return {
        ...
        "role_templates": playbook_manager.role_templates(),
    }
...
async def resolve_targets(
    body: PlaybookResolveTargetsRequest,
    session=Depends(auth_manager.get_current_session),
):
    try:
...
async def get_playbook(playbook_id: str, session=Depends(auth_manager.get_current_session)):
    try:
...
async def delete_playbook(playbook_id: str, session=Depends(auth_manager.get_current_session)):
    try:
