...
export function HostsOnboardingPage() {
  ...
  const handleDone = useCallback(() => {
    if (hosts.length === 0) {
      toast.error("Add at least one host first");
      return;
    }
        ...
        <div className="max-w-xl mx-auto space-y-4 border border-zinc-800 bg-zinc-900/30 p-6">
          <div className="space-y-1">
            <h1 className="text-zinc-100 font-semibold">
              Place hosts on a rack?
            </h1>
            <p className="text-sm text-zinc-500">
              You added {hosts.length} host{hosts.length !== 1 ? "s" : ""}.
    ...
    <div className="flex-1 min-h-0 overflow-auto p-6">
      <div className="max-w-xl mx-auto space-y-4 border border-zinc-800 bg-zinc-900/30 p-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-zinc-500">
            Add one or more hosts. Each host is a machine you can SSH into and
            run playbooks on. You can add more anytime from the Hosts page.
          </p>
        </div>
