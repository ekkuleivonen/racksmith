...
""")
    ...
    def test_extracts_racksmith_description(self, layout) -> None:
        layout.playbooks_path.mkdir(parents=True, exist_ok=True)
...
- name: X
  ...
  roles: []
  vars:
    racksmith_description: Full web stack deployment
""")
        pb = read_playbook(path, layout.repo_path)
        assert pb.description == "Full web stack deployment"
