"""Unit tests for the _run_ansible helper in worker/functions."""
...
from worker.functions import _run_ansible
...
_COMMON_KWARGS = dict(
    ...
    ansible_config_path="/tmp/ansible.cfg",
    roles_path=Path("/tmp/roles"),
    temp_files=[],
)
...
class TestRunAnsibleSuccess:
    async def test_streams_output_and_completes(self):
        process = FakeProcess(
        ...
        with patch("worker.functions.asyncio.create_subprocess_exec", return_value=process):
            await _run_ansible(redis=redis, **_COMMON_KWARGS)
    ...
    async def test_stores_exit_code_zero(self):
...
class TestRunAnsibleIdleTimeout:
    async def test_kills_process_on_idle_timeout(self):
        process = FakeProcess(stdout=FakeStdout(hang=True), exit_code=0)
        redis = _make_redis()

        with (
            patch("worker.functions.asyncio.create_subprocess_exec", return_value=process),
            patch("worker.functions.settings.ANSIBLE_IDLE_TIMEOUT", 0.1),
        ):
            await _run_ansible(redis=redis, **_COMMON_KWARGS)
    ...
    async def test_no_timeout_when_output_flows(self):
...
class TestRunAnsibleCommandNotFound:
    async def test_file_not_found_marks_failed_127(self):
        redis = _make_redis()

        with patch(
            "worker.functions.asyncio.create_subprocess_exec",
