"""Unit tests for auth/workspace, auth/session, auth/git pure utilities."""
...
from _utils.slugs import safe_slug
from auth.git import (
    get_file_diffs,
    get_file_statuses,
    get_racksmith_status_paths_repo_relative,
)
from auth.session import user_login, user_storage_id
...
class TestWalkTree:
    ...
    def test_returns_structure(self, tmp_path: Path) -> None:
        ...
        children = dir_entries[0]["children"]
        assert isinstance(children, list)
        assert len(children) == 1
        assert children[0]["name"] == "b.txt"
