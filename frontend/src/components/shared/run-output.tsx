...
interface RunOutputProps<T extends RunLike> {
  run: T | null;
  title?: (run: T) => string;
  emptyMessage?: string;
  onRunUpdate?: (run: T) => void;
  streamUrl: (runId: string) => string;
  fetchRun: (runId: string) => Promise<{ run: T }>;
}

export function RunOutput<T extends RunLike>({
  run,
  title,
  emptyMessage = "Run something to stream its output here.",
  onRunUpdate,
  streamUrl,
  fetchRun,
}: RunOutputProps<T>) {
  const [activeRun, setActiveRun] = useState<T | null>(run);
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket(streamUrl(activeRun.id));
    ...
    if (activeRun.status === "queued" || activeRun.status === "running") {
      pollInterval = window.setInterval(() => {
        void fetchRun(activeRun.id).then((res) => {
  ...
  }, [activeRun?.id, onRunUpdate, streamUrl, fetchRun]);
  ...
  const displayTitle = title ? title(activeRun) : activeRun.id;

  return (
    <section className="space-y-3 border border-zinc-800 bg-zinc-900/30 p-4">
      <div className="flex items-center justify-between gap-3">
        <div className="space-y-1">
          <p className="text-zinc-100 font-medium">{displayTitle}</p>
