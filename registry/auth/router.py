...
async def auth_exchange(body: ExchangeRequest) -> dict:
    """Client backend exchanges a one-time code for the GH token + user profile."""
    _cleanup_expired()
...
async def auth_refresh(
    ...
    """Return the stored GH token for a user (for session refresh).

    The client backend calls this when it needs a fresh token — e.g. the
    local session expired or the GH token was revoked and re-issued via
    a GitHub App refresh flow.
    """
