...
class TestReadRoleMetaMain:
    ...
    def test_parses_x_racksmith_in_options(self, layout) -> None:
        ...
        inp = role.inputs[0]
        assert inp.racksmith_label == "My Label"
...
class TestReadRoleActionYaml:
    ...
    def test_parses_action_yaml(self, layout) -> None:
        ...
        role = read_role(role_dir)
        assert role is not None
        assert role.slug == "install-packages"
        ...
        assert by_key["pkg"].racksmith_label == "Package"
...
class TestListRoles:
    ...
    def test_returns_parsable_roles(self, layout) -> None:
        for slug in ["a", "b", "c"]:
            ...
            role_dir.mkdir(parents=True, exist_ok=True)
            (role_dir / "action.yaml").write_text(
                yaml.safe_dump({"slug": slug, "name": slug.upper()})
            )
        roles = list_roles(layout)
        assert len(roles) == 3
        assert {r.slug for r in roles} == {"a", "b", "c"}
    ...
    def test_skips_dirs_without_manifest(self, layout) -> None:
        ...
        (layout.roles_path / "valid" / "action.yaml").write_text(
            yaml.safe_dump({"slug": "valid", "name": "Valid"})
        )
        (layout.roles_path / "invalid").mkdir()
        roles = list_roles(layout)
        assert len(roles) == 1
        assert roles[0].slug == "valid"
...
class TestWriteRole:
    ...
    def test_roundtrip(self, layout) -> None:
        original = RoleData(
            ...
            tags=["t1", "t2"],
            inputs=[
                RoleInput(key="x", type="str", racksmith_label="X", default="a"),
            ],
            has_tasks=False,
        )
        write_role(layout, original, tasks_yaml="- name: task\n  debug: {}\n")
        read_back = read_role(layout.roles_path / "roundtrip")
        assert read_back is not None
        assert read_back.slug == original.slug
        assert read_back.name == original.name
        assert read_back.description == original.description
        assert len(read_back.inputs) == 1
        assert read_back.inputs[0].racksmith_label == "X"
