...
from core.extensions import PREFIX, extract, inject, is_extension
...
class TestExtract:
    ...
    def test_strips_prefix_from_keys(self) -> None:
        ansible, racksmith = extract({"racksmith_rack": "r1", "racksmith_managed": True})
        assert "racksmith_rack" not in racksmith
        assert racksmith["rack"] == "r1"
        assert racksmith["managed"] is True
